...
async function fetchAuthorData(authorURL) {
  ...
  for (let i = 1; i <= 2; i++) {
    const element = document.querySelector(`div.profile-header__details > div > span:nth-child(${i})`);

    if (element && element.textContent.includes('Location'))
      location = element.textContent.replace(/[\n\r]+|[\s]{2,}/g, ' ').trim().split(' ').splice(2).join(' ');
    else if (element && element.textContent.includes('Joined on'))
      joinedOn = element.children[1].getAttribute('datetime').slice(0,10);

    const elementBottom = document.querySelector(`div.profile-header__bottom.fs-base > div:nth-child(${i})`);
    if (elementBottom && elementBottom.children[0].textContent.includes('Education'))
      education = elementBottom.children[2].textContent;
    else if (elementBottom && elementBottom.children[0].textContent.includes('Work'))
  ...
  return { name, image, location: location ?? '', joinedOn: joinedOn ?? '', website, education: education ?? '', work: work ?? '', '@id': id, '@type': 'devto:Person' };
}
...
async function fetchArticleData(articleURL) {
  ...
  return { name, description, author, image, url, datePosted, reactions, tags, '@id': `devto:${articleId}`, '@type': 'devto:Article' };
}
